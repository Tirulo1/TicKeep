    ...
    <main class="container my-4">
        ...
        <section class="title-section mb-4">
            <div>
                ...
                <p class="text-muted small mb-0"><?= count($garantias) ?>
                    <?= count($garantias) !== 1 ? $t['garantias_registradas'] : $t['garantia_registrada'] ?></p>
            </div>
            <div class="d-flex gap-2 flex-wrap">
                <div class="dropdown">
                    <div class="dropdown">
                        <button class="tk-btn-export d-flex align-items-center gap-1 dropdown-toggle" type="button"
                ...
                <a href="nueva-garantia.php"
                    class="tk-btn-primary text-decoration-none d-flex align-items-center gap-1">
                    <?= $t['nueva_garantia'] ?>
                </a>
            </div>
        </section>
        ...
        <ul class="nav tk-view-tabs mb-4" id="viewTabs" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="tk-view-tab active" id="list-tab" data-bs-toggle="tab" data-bs-target="#tab-lista"
            ...
            <li class="nav-item" role="presentation">
                <button class="tk-view-tab" id="cal-tab" data-bs-toggle="tab" data-bs-target="#tab-calendario"
                    type="button" role="tab">
                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor" stroke-width="2">
                ...
                <section id="garantias-list">
                    <?php if (count($garantias) > 0): ?>
                        <?php foreach ($garantias as $g): ?>
                            ...
                            ?>
                            <div class="tk-ticket-card" data-estado="<?= htmlspecialchars($status) ?>"
                                ...
                                <div class="ticket-info">
                                    ...
                                    <p class="mb-1 small"><?= $t['comprado_en'] ?> <span
                                    ...
                                    <?php endif; ?>
                                    <p class="ticket-expiry mb-0"><?= $t['vence_el'] ?>
                                        <b><?= fechaTickeep($g['fecha_vencimiento'], $preferencias); ?></b>
                                    </p>
                                ...
                                <a href="detalle.php?id=<?= $g['id_garantia']; ?>"
                                    class="tk-btn-details text-decoration-none"><?= $t['ver_detalles'] ?></a>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <div class="empty-state text-center py-5">
                            ...
                            <a href="nueva-garantia.php"
                                class="tk-btn-primary text-decoration-none mt-2 d-inline-block"><?= $t['anadir_primera'] ?></a>
                        </div>
                    <?php endif; ?>
            ...
            <div class="tab-pane fade" id="tab-calendario" role="tabpanel">
                <div class="calendar-legend mb-3 d-flex gap-3 flex-wrap">
                    <span class="legend-item"><span class="legend-dot"
                            style="background:#16a34a"></span><?= $t['vigente'] ?></span>
                    <span class="legend-item"><span class="legend-dot"
                            style="background:#d97706"></span><?= $t['expira_pronto'] ?></span>
                    <span class="legend-item"><span class="legend-dot"
                            style="background:#dc2626"></span><?= $t['caducada'] ?></span>
                </div>
    ...
    <script>
        ...
        function applyFilters() {
            ...
            cards.forEach(card => {
                ...
                const matchF = activeFilter === 'Todo'
                    || card.dataset.estado === activeFilter
                    || (activeFilter === 'Vigente' && card.dataset.estado === 'Expira pronto');
